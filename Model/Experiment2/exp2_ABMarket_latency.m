function  DSim=exp2_ABMarket_latency(latencyMag,latencyFrac,nPar)
...
addLatencyScenarioParam=@(X) addLatencyScenario(X,latencyFrac,latencyMag);
...
function addLatencyScenario(DSim,frac,latency)

aList=DSim.getAgentsByName('dsim.Comm');
for i=1:floor(length(aList)/frac)
    agent=aList{randi([1,length(aList)])};
    agent.Bandwidth=1e9;
